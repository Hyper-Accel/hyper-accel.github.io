...
지난번에 소개드린 GPU는 하나의 칩에 수백개의 개별 SM(Streaming Multiprocessor)가 존재하고, 각 데이터에 대한 프로그램을 thread로 정의하여 warp(32개 thread 묶음)로 묶어 실행 가능한 warp에 대한 프로그램을 순서에 상관없이 수행했습니다. CUDA kernel 레벨에서는 블록당 스레드 개수와 블록 개수만 지정하고, 그 하위 레벨(compiler, 하드웨어 scheduler 등)에서 어떤 하드웨어에서 해당 스레드(warp 단위로)가 실행될지 결정합니다. GPU의 명령 실행 기전은 warp별(32개의 데이터 포인트에 대한) 인스트럭션 실행(단일 연산 혹은 데이터 로드/스토어 등 개별 operation)이며, warp scheduler가 동적으로 warp를 선택하여 실행합니다.

하지만 TPU는 개별 칩에서 1~2개의 거대한 코어가 연산을 수행합니다. 또한 거대한 데이터를 thread나 warp단위로 나누어서 수행하는 대신 컴파일 타임에 **최적화된 데이터플로우**를 하드웨어에 직접 매핑하는 방식을 사용합니다. 연산을 개별 thread나 warp 단위로 바라보지 않고 tesor 단위로 바라보는 것입니다. 이를 위해 TPU는 여러 인스트럭션을 합쳐둔 더 고수준의 명령어(VLIW, Very Long Instruction Word)를 통해 더 많은 데이터에 대해서 한번에 연산 파이프라인에 밀어넣는 방식으로 동작하며, GPU와 달리 정적으로 연산 순서를 미리 정해두는 것이 특징입니다.
