...
최근 AI 가속기 시장에서 가장 핫하게 떠오른 키워드 중 하나는 바로 **TPU**일 것입니다. 지난 11월 구글은 7세대 TPU 아키텍쳐인 아이언우드(ironwood)를 소개하면서 이전 세대에 비해 향상된 학습 및 추론 성능과 클러스터 확장성, 그리고 AI 스타트업인 앤트로픽(Antrophic)과의 협업 등을 발표하였습니다. 국내/외 언론에서도 이를 관심있게 다루면서 일반 대중들에게도 TPU라는 용어가 알려지게 되었고, 이 뉴스 이후 구글과 엔비디아의 주가에도 적지 않은 영향을 주었습니다.
...
앞서 말씀드렸듯이 TPU는 AI 연산에 특화되어 제작되었습니다. AI 연산 중 가장 큰 특징은 대규모의 행렬곱셈 수행한다는 점인데요. 이는 모델 종류(CNN, RSTM, Transformer)에 관계 없이 학습과 추론 과정에서 인공 신경망의 가중치를 사용하고 기타 다른 값들을 연산하는 과정에서 볼 수 있는 공통적인 특징입니다.
...
다음으로는 TPU 개별 칩에서 사용하는 core architecture에 대해 알아보겠습니다. 지난번에 소개드린 GPU는 하나의 칩에 수백, 수천개의 개별 core가 존재하고 이를 소프트웨어에서 여러개의 thread로 컨트롤합니다. 하지만 TPU는 3~6개 정도로 적은 수의 거대한 코어로 연산을 수행합니다.   
...
여러 개의 칩을 연결하여 하나의 거대한 서버를 구축하는 scale-up을 위해 TPU는 torus topology를 사용합니다. 여러 개의 칩을 연결한 모습을 떠올려보면 일반적으로 첫번째 그림과 같은 바둑판 모양의 격자 구조 (mesh topology)를 생각해볼 수 있는데요. 이 구조의 단점은 양끝에 있는 노드들끼리 연결이 안되어 있기 때문에 노드간 최대 거리가 길어지나든 점입니다. torus 구조에서는 양 끝에 있는 node들을 wrap-around link로 연결하여 배선 복잡도는 높아지지만 노드간 최대 거리를 N → N/2로 줄일 수 있습니다. 4세대 TPU에서는 이 torus 구조를 3D로 확장하여 노드간 통신 범위와 하나로 묶이는 TPU 개수를 확장하였습니다. 4 * 4 * 4(=64)개의 TPU 묶음을 **cube**라고 부르고 이 cube가 여러 개 모인 클러스터를 **pod**이라고 부릅니다. 

**Optical circuit switch(OCS)**

![twisted torus example](twisted_torus.webp)

이러한 칩 간 통신을 중재 하기 위해서는 네트워크 스위치가 필요한데요. TPU는 광학 스위치인 OCS를 도입하여 torus 간의 연결 구조를 소프트웨어에서 재구성할 수 있도록 하였습니다. 연산 특성에 따라 최적화된 연결 구조를 소프트웨에서 결정할 수 있도록 한 것입니다.
...
구글은 이번세대 TPU부터 과감하게 TensorFlow 지원을 중단했습니다. 이는 PyTorch의 생태계 지배력을 인정하면서 PyTorch 생태계 내에서(PyTorch/XLA) TPU의 성능을 최적화 할 수 있도록 **기존 생태계에 XLA를 통합**하고,독자적인 framework 개발은 TensorFlow보다 성능 향상에 더 최적화된 **JAX framework에 더 집중**하겠다는 두가지 의미로 해석됩니다.
