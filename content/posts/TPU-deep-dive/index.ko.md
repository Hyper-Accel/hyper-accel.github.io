...
하지만 TPU는 개별 칩에서 1~2개의 거대한 코어가 연산을 수행합니다. 또한 거대한 데이터를 thread나 warp단위로 나누어서 수행하는 대신 컴파일 타임에 **최적화된 데이터플로우**를 하드웨어에 직접 매핑하는 방식을 사용합니다. 연산을 개별 thread나 warp 단위로 바라보지 않고 tensor 단위로 바라보는 것입니다. 이를 위해 TPU는 여러 인스트럭션을 합쳐둔 더 고수준의 명령어(VLIW, Very Long Instruction Word)를 통해 더 많은 데이터에 대해서 한번에 연산 파이프라인에 밀어넣는 방식으로 동작하며, GPU와 달리 정적으로 연산 순서를 미리 정해두는 것이 특징입니다.
