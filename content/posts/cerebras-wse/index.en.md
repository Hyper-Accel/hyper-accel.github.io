...
Their core concern was fairly clear. In conventional chip designs, on-chip memory capacity is limited by chip size. Larger models also require more links between devices. As a result, memory bandwidth and inter-chip communication easily become bottlenecks.
...
By contrast, **WSE-3** places memory directly on the chip. It distributes **Static Random-Access Memory(SRAM)** across the cores. Public WSE-3 materials describe **48KB of SRAM** per core, which adds up to **44GB** of on-chip memory across the wafer. In Cerebras's framing, moving from HBM to SRAM delivers **more than an 800x increase in memory bandwidth**. Because the die regions are also kept together on a single chip, communication between them happens over very short on-silicon wires, which Cerebras says can provide **up to 33x faster inter-die communication** than an H100-based design.
...
**WSE** operates in a dataflow style, where computation happens when data arrives. Weights streamed from MemoryX are turned into packets in the form of `(index, value)` and routed to the cores that need them. When a core receives a packet, it reads the activation that matches that index and performs the multiply-add.

This packet-oriented structure is what makes WSE effective even for unstructured sparsity. The key point is that zero-valued weights are never turned into packets in the first place. That means they do not create fabric traffic, and they do not trigger any corresponding computation.

In other words, using sparsity in WSE is not just a matter of compressing zeros in storage. It means the system avoids generating both data movement and compute events for zero weights altogether. The WSE-2 paper argues that this fine-grained dataflow execution model can deliver higher real utilization on unstructured sparsity than a GPU's dense tile-based execution model.
...
![Scalability of the CS-3 system](images/16-single-cs-3-and-scaleout-to-2048.png)
...
![CS-3 server](images/17-cs-3-breakdown.png)
...
![Cerebras versus GPU inference performance comparison](images/18-inference-performance.png)
...
![Llama70B inference performance comparison between Cerebras and DGX-H100 under a short-output setting](images/19-short-inference-performance.png)
...
There is no official price list, but public estimates help show the scale. The Next Platform estimates a CS-2/CS-3 node at roughly **$2.5M**–**$3.2M**, or around **\$4M** when MemoryX and SwarmX are included. SEC-filed purchase orders also show CS-3 cluster goods fees in the **\$178M**–**\$350M** range.

In other words, moving toward WSE is not just a matter of chip performance. It implies large upfront deployments. Once CS-3, MemoryX, SwarmX, networking, power, and cooling are bundled together, the required investment can quickly climb into the **hundreds of millions of dollars**.
