...
On top of that, HBF's weaknesses are also NAND Flash's weaknesses. HBF was proposed as a way to overcome HBM's capacity limits, but because it is a Flash-based alternative, we need to see where and how Flash memory is actually used in LLM serving today. Only then can we tell what differentiates HBF from existing Flash memory.
...
One of the conditions Part 2 identified for hiding HBF's weaknesses was: **if the workload is deterministic, prefetching can hide the latency.** The H³ evaluation rests on exactly this assumption — that we can predict which weights and which KV cache entries the next layer will read.
...
The biggest problem with the attention mechanism — the most important operation in Transformer models — is that compute grows quadratically with input length. **Mamba** and other linear-attention approaches have been studied to overcome this, but their performance ceiling against Transformers remains clear; they are typically used only as part of hybrid stacks combined with full attention.

Recent frontier research therefore reduces attention compute by using only a subset of the accumulated Key-Value pairs, or by using compressed versions of them, lowering both compute and memory traffic. The underlying intuition is that even though attention conceptually relates every pair of tokens, in practice only a small set of tokens truly matter for predicting the next one. Pictured:
...
The catch is that, to make this picture real, HBF has to confront the inherent weaknesses of NAND. Part 2 covered NAND's most painful one — latency — and the ways to route around it. But there is more left to solve beyond latency. The next section walks through them.
