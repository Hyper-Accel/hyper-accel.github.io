...
하지만 “AI로 생산성을 올려보자”라는 우리 팀의 엔진에 진짜 로켓 부스터를 단 건 Claude Code였습니다. Claude Code는 skills(스킬)이라는 개념을 도입했는데, 이는 정적인 rules를 훨씬 뛰어넘는 **구조화된 다단계 워크플로우**입니다. 저희 팀 레포 중 하나에서는 이제 `.claude` 디렉토리를 두고, 계획 수립, 기능 구현, 테스트 작성, PR 오픈, PR 리뷰 코멘트 가져와 반영하기 같은 스킬들을 관리하고 있습니다. 또한 플랫폼에 특화된 하드웨어 제약, 팀이 ‘피를 보고’ 배운 안티패턴, SDK별 컨벤션 같은 것도 스킬 또는 claude code rules 로 녹여두었죠.

룰 파일과 스킬의 차이는, 스타일 가이드와 “훈련된 동료”의 차이와 비슷합니다.
룰은 “pytest를 써라, unittest는 쓰지 마라”라고 말합니다.
스킬은 “이 유형의 모듈에 맞는 테스트 스위트를 작성하고, 실행하고, 실패하면 최대 다섯 번까지 자동 수정하고, 결과를 요약 보고하라”라고 말합니다. `/implement-tested`, `/pr-review`, `/open-pr` 같은 스킬은 과거에 몇 시간이 걸리던 작업들을, 인간의 감독 하에 **몇 분 단위의 작업**으로 바꿔놓았습니다. 더 디테일한 내용은 다음 글에서 이야기 하도록 하겠습니다.
...
저희 팀(그리고 HyperAccel의 다른 팀들)이 AI가 워크플로우에 녹아드는 새로운 방법들을 계속 찾아가는 과정에서, “코드를 개발하는 방식”이 바뀌는 이 새로운 패러다임 속을 어떻게 항해하고 있는지 계속 공유하겠습니다.
