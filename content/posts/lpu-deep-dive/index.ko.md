...
draft: false
title: '지피지기면 백전불태 3편 : 엔비디아가 30조에 인수한 그록의 LPU'
cover:
...
# 지피지기면 백전불태 3편 : 엔비디아가 30조에 인수한 그록의 LPU

> **"상대를 알고 나를 알면 백 번 싸워도 위태롭지 않다."**  
> 이 시리즈는 AI 가속기 설계를 위해 경쟁사들의 하드웨어를 깊이 이해하는 것을 목표로 합니다.  
> 세 번째 글에서는 작년 말 엔비디아에 약 20억달러로 인수된 미국의 스타트업 그록(groq)의 **LPU**(Language Processing Unit)에 대해 다룹니다.
...
- **GPU / TPU:** 학습(Training)과 추론(Inference)을 모두 커버하지만, 태생적으로 **학습(대규모 병렬 연산)**에 최적화됨.
- **LPU:** 학습은 과감히 버리고, 오직 **거대언어모델(LLM)의 추론 최적화**에만 올인한다.
...
컴퓨터 구조에서는 하드웨어 성능의 한계를 진단하기 위해 **Roofline Model**이라는 측정 기법을 사용합니다. 하드웨어에서 사용되는 소프트웨어의 연산 특성과 하드웨어 스펙을 통해 해당 연산의 가능한 최대의 성능을 확인하는 것입니다. x축에 해당하는 값은 **연산강도**입니다. 한번 가져온 데이터로 얼마 만큼의 연산을 수행하느냐를 나타내는 지표이며, 데이터 재사용성이 높아질수록 증가합니다. 연산강도가 충분히 높아서 위 그래프의 미분 불가능 지점에 도달하게 되면 해당 연산의 성능을 하드웨어가 낼 수 있는 최대 연산 성능과 같아집니다.(**Compute bound**) 반면 연산 강도가 낮으면 하드웨어 성능이 아무리 좋아도 메모리 대역폭 만큼의 성능만 뽑아낼 수 있게 됩니다.(**Memory bound**) 

- **Compute Bound :** 하드웨어 스펙에 비해 필요로 하는 연산량이 너무 많아 칩의 계산 속도가 병목
...
GPU와 다른 데이터센터향 칩들이 비싼 HBM(고대역폭 메모리)을 사용해야만 하는 이유가 바로 여기에 있습니다. 아무리 칩의 연산 속도가 빨라도, 메모리 벽(Memory Wall)을 넘지 못하면 무용지물이기 때문입니다. 메모리 대역폭을 늘리게 되면 아래 그래프와 같이 같은 연산 강도에서도 높은 성능을 뽑아낼 수 있습니다.
...
## 그록의 과감한 도박 : Memory Hierarchy를 붕괴시켜라
...
Nvidia를 비롯한 대기업들과 많은 추론 칩 스타트업들이 HBM과 메모리를 더 효율적으로 쓸 수 있는 방법을 고민하였지만, 그록은 이러한 memory hierarchy에 의문을 품고 판을 뒤집는 질문을 던집니다.
...
- **Tensor Parallelism (TP):** 하나의 행렬 연산을 여러 칩이 나누어 처리. 부분적 행렬 연산에 필요한 가중치를 여러 칩의 메인 메모리에 나눠서 저장
- **Pipeline Parallelism (PP):** 모델의 레이어(Layer)를 그룹 별로 나누어 처리. 연산 순서 별로 각 칩은 이전 칩에서 연산된 결과를 받아서 다음 레이어에 대한 연산을 수행.

![gpu example](gifs/gpu_example.gif)

![lpu example](gifs/lpu_example.gif)

그록은 이렇게 만든 그들의 시스템을 거대한 **컨베이어 벨트**에 비유합니다. 입력 데이터가 첫 번째 칩(벨트의 시작)에 들어가면, 각 칩은 본인이 맡은 연산만 수행하고 옆 칩으로 데이터를 넘깁니다. 마지막 칩에서 최종 토큰이 튀어나오는 구조입니다. GPU의 단일 칩에서 같은 작업을 수행하려 한다면 

> HBM에서 memory load -> 연산 -> 다시 HBM에서 memory load -> 연산

이 과정을 반복해야 합니다. HBM에 연산에 필요한 데이터들은 저장하고 있지만, 연산기 근처(SRAM)에 load할 수 있는 데이터의 용량에는 마찬가지로 한계가 있기 때문입니다. 이로 인해 반복적으로 memory load가 이뤄지는 과정에서 병목이 발생하는 것이 그록의 주장입니다.

이 구조가 성공하려면 **칩 간 통신(Chip-to-Chip, C2C)** 속도가 생명입니다. 컨베이어 벨트 중간이 끊기면 공장 전체가 멈추니까요. 그록은 독자적인 **C2C 인터커넥트** 기술과 **Dragonfly Topology**를 통해 8개 이상의 칩을 하나의 거대한 노드처럼 묶었습니다. 여러 개의 칩이 서로의 SRAM을 공유하는 거대한 단일 칩처럼 보이게 하여 통신 오버헤드(Communication Overhead)를 거의 0에 수렴하게 만든 것입니다.
...
**(지금부터 설명하는 예시는 단순한 설명을 위한 것이며, 실제 하드웨어 동작과는 다를 수 있습니다.)**
...
그렇다면 엔비디아가 이 그록을 인수한 배경은 무엇일까요? 단순한 추론 특화 칩 기술력을 구매하기 위한 것이라고 보기는 어렵다고 생각합니다. 제가 생각해본 그록 인수 배경을 몇 가지 이야기해보며 글을 마무리하도록 하겠습니다. 
...
**(이 부분에서는 제 주관과 그록의 입장이 많이 반영되어 있습니다.)**
...
- **GPU** : 복잡한 논리 추론, 코드 생성, 이미지 렌더링 등 막대한 연산이 필요한 무거운 작업 담당합니다.
