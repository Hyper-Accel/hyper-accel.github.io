...
1. LLM이 non-deterministic 하기 때문에 LM 프로그램을 프로그래밍하기 어렵습니다.
...
여기까지 읽으셨다면, 요청 순서가 최적이 아니어서 캐시 성능이 저하되면 어떻게 될까 궁금하실 수 있습니다. 저자들도 이를 고려했습니다. 성능을 최대화하기 위한 캐시 인식 스케줄링 알고리즘을 설계했습니다. 이 알고리즘은 radix-tree KV cache와 가장 잘 일치하는 접두사 길이로 요청을 정렬합니다(본질적으로 들어오는 요청을 처리하기 전에 캐시 적중률을 사전 평가합니다).

## Compressed FSM을 사용한 constrained decoding
...
성능 향상은 Radix attention과 FSM 기반 디코딩에서 비롯됩니다. 논문 저자들에 따르면 최고 성능을 보인 DsPy RAG 파이프라인에서 캐시 적중률이 50-90% 사이였다고 합니다.
