...
첫째, **캐시 일관성(cache coherency)이 없습니다.** 캐시 일관성이란 여러 주체(CPU 코어, 디바이스)가 같은 메모리 주소를 각자 캐싱하더라도, 한쪽이 값을 바꾸면 나머지도 항상 최신 값을 보도록 보장하는 성질입니다. CPU가 PCIe 장치의 메모리를 직접 load/store 하려면 이 일관성이 캐시 라인 단위로 보장되어야 합니다. PCIe는 본래 패킷 기반 I/O 프로토콜이라 이 보장이 없습니다. PCIe 메모리를 메인 메모리처럼 쓰려면 OS가 명시적으로 데이터를 옮겨주는(memcpy) 방식이 필요합니다.
