...
> [1편](https://hyper-accel.github.io/posts/what-is-hbf/), [2편](https://hyper-accel.github.io/posts/hbf-workload/), [3편](https://hyper-accel.github.io/posts/hbf-challenge/) 에서는 GPU 옆 메모리 계층의 빈 자리를 채우는 **HBF** 를 다뤘습니다.
> 이번 편부터는 시점을 GPU 바깥, 시스템 레벨로 옮겨서 **CXL(Compute Express Link)** 이 메모리 계층의 어디를 채우는지 살펴봅니다.
...
LLM 추론 서비스가 한 노드에서 수십 TB의 KV cache, 모델 weight, 임베딩을 다루기 시작하면서, 이 천장에 닿는 상황이 점점 잦아지고 있습니다.
...
그런데 PCIe로 메모리를 붙이면 두 가지 문제가 생깁니다.
...
<!-- TODO: 그림 #1 — DDR/PCIe/CXL 위치 비교 다이어그램 (메모리 계층 피라미드 + CXL이 채우는 자리) -->

이 빈 자리를 채우기 위해 등장한 표준이 바로 **Compute Express Link(CXL)** 입니다.
...
<!-- TODO: 그림 #2 — CXL.io / CXL.cache / CXL.mem 세 sub-protocol의 데이터 흐름 (호스트↔디바이스 화살표) -->
...
여기서 중요한 점은 **CPU가 home agent 역할을 한다** 는 것입니다. Type 3처럼 디바이스가 메모리만 들고 있는 경우, 호스트가 단독으로 일관성을 관리하면 충분합니다.
...
<!-- TODO: 그림 #3 — CXL 1.1 → 2.0 → 3.x 토폴로지 진화 (단일 호스트 → 스위치 + 풀링 → 패브릭 + 멀티 호스트 공유) -->
...
2.0에서 결정적인 변화가 일어납니다. **CXL 스위치** 가 도입되어 여러 호스트와 여러 메모리 디바이스를 그물처럼 엮을 수 있게 됩니다.

가장 인상적인 응용은 **메모리 풀링(memory pooling)** 입니다. 데이터센터의 메모리 활용률은 평균적으로 50% 안팎입니다. 어떤 서버는 메모리가 모자라고 어떤 서버는 절반 이상 비어 있는 상황이 흔하죠.

CXL 2.0에서는 **여러 호스트가 하나의 메모리 풀에서 필요한 만큼 동적으로 할당받을 수 있습니다.**

한 가지 주의할 점은, 이때의 "공유"가 **동시 공유가 아니라 동적 할당** 이라는 것입니다. 거대한 메모리 디바이스가 **Multi-Logical Device(MLD)** 형태로 여러 chunk로 쪼개지고, **Fabric Manager(FM)** 라는 컨트롤 플레인이 "이 chunk는 호스트 A에게, 저 chunk는 호스트 B에게" 식으로 분배합니다. 한 chunk는 시점상 한 호스트가 단독 소유하므로, **캐시 일관성 충돌 자체가 발생할 영역이 없습니다.**
...
둘째, **포트 단위 연결에서 패브릭(fabric) 연결로.** 3.x는 다수의 CXL 스위치를 엮어 큰 규모의 패브릭을 구성하고, **PBR(Port-Based Routing)** 같은 기능으로 mesh·dragonfly·3D torus 같은 다양한 토폴로지로 디바이스를 배치할 수 있게 합니다.

이 단계까지 오면 CXL은 사실상 **데이터센터 내부의 새로운 인터커넥트 표준** 으로 자리잡습니다. NVIDIA의 NVLink, UALink 같은 가속기 인터커넥트와도 비교 대상이 되기 시작하죠.
...
**메모리 모듈 안에 연산 엔진을 넣기.** SK하이닉스의 **CMM-Ax** 가 대표적이고, 삼성도 **CMM-H** 와 **CXL-PNM(Processing-Near-Memory)** 으로 비슷한 그림을 그립니다.
...
2편에서 다룬 HBF + HAVEN 조합("HBF 옆에 search 엔진을 둔다")과 같은 발상이 CXL 영역에서는 NMP 통합 모듈로 구체화되고 있는 셈입니다.

### 패브릭과 IP 진영 — 컨트롤러부터 스위치 실리콘까지

메모리 3사 외에도 CXL 생태계에서 빠지지 않는 플레이어가 있습니다. **CXL 컨트롤러·스위치 IP를 공급하거나, 자체 실리콘으로 직접 만드는** 회사들입니다.

- **Astera Labs**: Leo 시리즈 컨트롤러로 **Micron CZ120 같은 메모리 모듈, Microsoft Azure M-series CXL 메모리** 등에 채택되어 있습니다. CXL 메모리 모듈의 "표준 부품" 같은 위치를 차지하고 있죠. (반면 Samsung CMM-D는 자체 컨트롤러를 사용해, 메모리 회사가 자사 SoC를 함께 가져가는 경우도 있습니다.)
- **Marvell**, **Microchip(구 Microsemi)**: 컨트롤러 라인업으로 IP 진영의 한 축을 차지합니다.
- **Panmnesia(파네시아)**: KAIST **CAMELab(Computer Architecture and Memory Expansion Lab)** 스핀오프인 한국 fabless 스타트업입니다. 흥미롭게도, 앞서 인용한 *Memory Pooling With CXL* 논문의 저자 M. Jung이 바로 이 회사의 창업자이신 **정명수 KAIST 교수님** 이십니다. 단순한 컨트롤러를 넘어 [**CXL 3.2 fabric switch 실리콘(PANSWITCH)**](https://panmnesia.com/) 까지 직접 만드는 풀스택 플레이어로, **PBR**, mesh·dragonfly·3D torus 토폴로지 지원 등 3.x의 최첨단 기능을 가장 먼저 silicon으로 구현하고 있습니다. 2026년 4월 양산 발표로 본격적인 사업화 단계에 들어섰습니다.
...
- **남는 숙제**: 2-3배의 latency tax, 소프트웨어 스택의 미성숙, Type 2의 빈자리.
...
- [Panmnesia PANSWITCH — CXL 3.2 Fabric Switch (mass production, Apr 2026)](https://panmnesia.com/)
- [Linux Kernel CXL Documentation](https://docs.kernel.org/driver-api/cxl/index.html)
- D. Gouk, S. Kwon, M. Bae, M. Lee, M. Jung, "Memory Pooling With CXL," *IEEE Micro*, vol. 43, no. 2, pp. 48-57, 2023. DOI: [10.1109/MM.2023.3237491](https://doi.org/10.1109/MM.2023.3237491)
