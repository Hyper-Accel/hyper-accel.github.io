...
# simple_matmul 호출 시점에 JIT 컴파일되어 코드가 실행되며, 이미 컴파일된 바이너리가 있다면 코드가 변경되지 등은 이상 cache된 바이너리를 사용합니다.
...
하드웨어 제약상 텐서가 완전히 동적일 수는 없습니다. 모든 동적 차원은 버퍼 할당과 명령 생성을 위한 최댓값을 가져야 하며, 상한이 없는 동적 차원은 컴파일 시점에 에러를 발생시킵니다. 이는 컴파일러가 컴파일 타임에 tensor allocation을 planning하기 위함입니다.
